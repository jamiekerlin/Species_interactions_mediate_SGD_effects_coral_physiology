---
title: "Species interactions mediate the effects of submarine groundwater discharge
on coral physiology"
author: "Jamie Kerlin"
date: '2022-05-31'
output: 
...
knitr::opts_chunk$set(echo = TRUE, #we want to show our script, so keeping this TRUE
                      ...
                      warning = FALSE,  
                      fig.path = "../Output/Physiology/") 


```

## Notes for this script
- Add in CC/cell data here so all one script
- Make sure PR code can run with what is currently in the folders
...
ggplot(data_wide, mapping = aes(x = Initial, y = Final)) +
  geom_point() +
  geom_smooth(method = "lm") +
  facet_wrap(~Response_measurement, scales = "free") +
  theme_bw()

initial_final <- data_wide %>%
...
df2 <- results_initial_final %>%
  ...
  mutate(across(Response_measurement, factor, levels = c("CCcm2", "ED", "GP", "R"),
                labels = c("Chlorophyll Content (cm-2)", "Endosymbiont Density", 
                           "Gross Photosynthesis", "Respiration"))) %>%
  mutate(Significance = case_when(p > 0.05 ~ "",
                                  p < 0.05 & p > 0.01 ~ "*",
                                  p < 0.01 & p > 0.001 ~ "**",
                                  p < 0.001 ~ "***"))
...
gp_nn_results_cleannames1 <- gp_nn_results_cleannames %>%
  ...
  mutate(low = estimate - std.error * 1.96, high = estimate + std.error*1.96) %>%
  mutate(alphasig = if_else(p.value <= 0.05, 1, 0))
...
results_r_nn_cleannames1 <- results_r_nn_cleannames %>%
  ...
  mutate(alpha = if_else(p.value <= 0.05, 1, 0))
...
ungroup() %>% # ungroup the data
  nest(data = -c(Response_measurement, Predictor_measurement)) %>% # nest all the data measurement type
  mutate(fit = map(data, ~lm(Final~Initial + Predictor_scaled, data = .))) #create models
cc_nn_ms
...
cc_plot_control1 <- ggplot(CC_plot_control, mapping = aes(x = range_pH, y = Final)) +
  ...
  theme(axis.text = element_text(size = 14),
        ...
        axis.title.x = element_markdown(size = 18),
        axis.title.y = element_markdown(size = 14)) +
  labs(x = "Range pH", y = "Final CC cm<sup>-2</sup>")

cc_plot_control1

cc_plot_control2 <- ggplot(CC_plot_control, mapping = aes(x = range_pH, y = Final)) +
  geom_point(CC_plot1, mapping = aes(x = range_pH, y = Final, color = Treatment), alpha = 0.7, size = 3) +
  geom_smooth(method = "lm", color = "#556385") +
  theme_bw() + 
  scale_color_manual(values = colors) +
  theme(axis.text = element_text(size = 14),
...
```{r residual_cc}
#create first model for only the no neighbor treatment
control_cc_m <- lm(Final ~ Initial + range_pH, data = CC_plot_control)
control_cc_m2 <- lm(Final ~ range_pH, data = CC_plot_control)

summary(control_cc_m)
glance(control_cc_m)
summary(control_cc_m2)
glance(control_cc_m2)

AIC(control_cc_m, control_cc_m2)

summary(test <- lm(Final ~ Initial, data = CC_plot)) ### they are correlated
ggplot(data = CC_plot_control, mapping = aes(x = Initial, y = Final)) +
  geom_point() +
  geom_smooth(method = "lm")
...
a_cc <- as.numeric(exp_summ_cc %>% filter(term == "range_pH") %>% select(estimate)) #slope of del15N
...
CC_plot_pred <- CC_plot %>%
  mutate(predict = (a_cc*range_pH) +  (b_cc*Initial) + c_cc, #calculate expected value
         resid = Final - predict) %>% #calculate residuals (obs - exp)
  group_by(Treatment) %>% #group by treatment 
  mutate(meanresid = mean(resid))  
...
cc_plot_resid <- ggplot(tidy_cc1, mapping = aes(x = Treatment, y = estimate)) +
  ...
  geom_jitter(mapping = aes(x = Treatment, y = resid, color = Treatment), size =2,  alpha = 0.7, width = 0.1) +
  geom_errorbar(mapping = aes(ymax = high, ymin= low), width=0.1) +
  labs(y = "Chlorophyll a content observed-expected  (mg cm<sup>-2</sup>)",
                  x = "Neighbor Treatment") +
  theme_bw() +
  scale_color_manual(values = colors) +
  geom_hline(yintercept = 0, lty = 2) +
...
## Check model assumptions
```{r checkmodel_cc}
check_model(control_cc_m2)
check_model(comp_cc_m) #7h is outlier
...
ggplot(mapping = aes(x = reorder(Predictor, -delAIC), y = delAIC)) +
  ...
  theme_classic() +
  geom_hline(yintercept = 2, lty = 2) +
  labs(x = "", y = "", title = "Chlorophyll a Content (surface area normalized)") +
    theme(axis.text = element_text(size = 10),
        legend.text = element_text(size = 12),
        legend.title = element_text(size = 12),
        strip.text = element_text(size = 12),
...
cc_plot_control1 <- ggplot(CC_plot_control, mapping = aes(x = range_pH, y = Final)) +
  ...
  theme(axis.text = element_text(size = 8),
        ...
        axis.title.x = element_markdown(size = 10),
        axis.title.y = element_markdown(size = 9)) +
  labs(x = "pH Range", y = "Chlorophyll         a Content (µg cm<sup>-2</sup>)")
...
cc_plot_resid <- ggplot(tidy_cc1, mapping = aes(x = Treatment, y = estimate)) +
  geom_jitter(mapping = aes(x = Treatment, y = resid, color = Treatment), size =1.5,  alpha = 0.5, width = 0.1) +
    geom_point(size = 2.5) +
  geom_errorbar(mapping = aes(ymax = high, ymin= low), width=0.2) +
  labs(y = "",x = "", title = "Chlorophyll a Content (surface area normalized)") +
  theme_bw() +
  scale_color_manual(values = colors) +
  geom_hline(yintercept = 0, lty = 2) +
  theme(axis.text.y = element_text(size = 8),
...
cc_table <- cc_nn_results_cleannames %>%
  select(Predictor, term, estimate, std.error, statistic, p.value, r.squared, adj.r.squared, AIC) %>%
  mutate(Response = "Chlorophyll a Content (cm-2)", 
         delAIC = AIC - min(AIC)) %>%
  arrange(delAIC) %>%
  select(Response, everything()) 
...
comp_cc_summ <- tidy(comp_cc_m_table) %>% mutate(Response = "Chlorophyll a Content (cm-2)") %>%
  mutate(across(term, factor, levels = c("(Intercept)", "TreatmentDead_skeletal",
                                         "TreatmentConspecific", "TreatmentHeterospecific"),
                labels = c("(Intercept)", "Dead Skeletal", "Conspecific", "Heterospecific")))
...
comp_cc_m_r2 <- as.data.frame(r.squaredGLMM(comp_cc_m_table)) %>%
  mutate(Observations = 58,
         ...
         Response = "Chlorophyll a Content (cm-2)") %>%
  mutate(`Marginal R-squared` = round(R2m, 2),
         `Conditional R-squared` = round(R2c, 2)) %>%
  select(Response, Observations, everything())
...
coeff_cc <- cc_nn_results_cleannames1 %>%
  mutate(Response_measurement = "CC cm-2")%>%
  select(Response_measurement, Predictor, estimate, low, high, p.value, delAIC, alpha)
